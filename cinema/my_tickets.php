...
try {
    ...
    $danhSachVe = $cauTruyVan->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    $loi = 'Lỗi khi tải vé: ' . $e->getMessage();
}
?>
<!DOCTYPE html>
<html lang="vi">
...
<body>
    ...
    <main class="container" style="padding: 40px 16px;">
        ...
        <?php if ($loi): ?>
            <div class="alert alert-error" style="margin-top: 16px;"><?php echo htmlspecialchars($loi); ?></div>
        <?php endif; ?>
        ...
        <?php else: ?>
            <div style="display:grid; gap:16px; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); margin-top: 20px;">
                <?php foreach ($danhSachVe as $ve): ?>
                    <?php 
                        $mauTrangThai = '#6c757d';
                        if ($ve['TrangThaiVe'] === 'ChuaSuDung') $mauTrangThai = '#17a2b8';
                        if ($ve['TrangThaiVe'] === 'DaSuDung') $mauTrangThai = '#28a745';
                        if ($ve['TrangThaiVe'] === 'Huy') $mauTrangThai = '#dc3545';
                    ?>
                    <div style="background:#fff; border:1px solid #eee; border-radius:12px; overflow:hidden;">
                        <div style="padding:16px; border-bottom:1px dashed #eee; display:flex; justify-content:space-between; align-items:center;">
                            <div style="font-weight:600; color:#1a1a1a;"><?php echo htmlspecialchars($ve['TenPhim']); ?></div>
                            <?php if ($ve['TrangThaiVe'] !== 'ChuaSuDung'): ?>
                            <span style="padding:4px 10px; border-radius:16px; color:#fff; background: <?php echo $mauTrangThai; ?>; font-size:.8rem; font-weight:600;">
                                <?php echo htmlspecialchars($ve['TrangThaiVe']); ?>
                            </span>
                            <?php endif; ?>
                        </div>
                        <div style="padding:16px; color:#444;">
                            <div><strong>Suất:</strong> <?php echo date('d/m/Y H:i', strtotime($ve['ThoiGian'])); ?></div>
                            <div><strong>Phòng:</strong> <?php echo htmlspecialchars($ve['TenPhong']); ?> (<?php echo htmlspecialchars($ve['LoaiPhong']); ?>)</div>
                            <div><strong>Ghế:</strong> <?php echo htmlspecialchars($ve['SoGhe']); ?></div>
                            <div><strong>Giá vé:</strong> <?php echo number_format($ve['GiaVe']); ?>đ</div>
                            <div style="margin-top:8px; font-size:.9rem; color:#666;">
                                Mua lúc: <?php echo date('d/m/Y H:i', strtotime($ve['NgayMua'])); ?>
                            </div>
                        </div>
