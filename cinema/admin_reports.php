...
try {
    $stmt = $conn->prepare("
        SELECT 
            DATE(sc.ThoiGian) as Ngay,
            COUNT(v.MaVe) as SoVe,
            SUM(v.GiaVe) as DoanhThu,
            COUNT(DISTINCT v.MaKH) as SoKhachHang
        FROM SUATCHIEU sc
        LEFT JOIN VE v ON sc.MaSuat = v.MaSuat AND v.TrangThai != 'Huy'
        WHERE DATE(sc.ThoiGian) BETWEEN ? AND ?
        GROUP BY DATE(sc.ThoiGian)
        ORDER BY Ngay DESC
    ");
...
try {
    $stmt = $conn->prepare("
        SELECT 
            ...
            SUM(v.GiaVe) as DoanhThu
        FROM PHONG ph
        LEFT JOIN SUATCHIEU sc ON ph.MaPhong = sc.MaPhong
        LEFT JOIN VE v ON sc.MaSuat = v.MaSuat AND v.TrangThai != 'Huy'
        WHERE DATE(sc.ThoiGian) BETWEEN ? AND ?
...
<div class="stats-grid">
    ...
    <div class="stat-card">
        ...
        <div class="stat-card-value"><?php echo count($revenueStats); ?></div>
    </div>
</div>
